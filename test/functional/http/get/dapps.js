		...
		describe('sort=', function () {
			...
			it('using "name:asc" should return result in ascending order', function () {
				return dappsEndpoint.makeRequest({sort: 'name:asc'}, 200).then(function (res) {
					_(res.body.data).map('name').map(_.toLower).sort().value().should.eql(_.map(res.body.data, 'name').map(_.toLower));
				});
			});
			...
			it('using "name:desc" should return result in descending order', function () {
				return dappsEndpoint.makeRequest({sort: 'name:desc'}, 200).then(function (res) {
					_(res.body.data).map('name').map(_.toLower).sort().reverse().value().should.eql(_.map(res.body.data, 'name').map(_.toLower));
				});
			});
		});
